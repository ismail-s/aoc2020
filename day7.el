...
(setq day7-input-example "light red bags contain 1 bright white bag, 2 muted yellow bags.
...
(setq day7-input "plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
...
(defun day7-parse-bag-rules (input-lines)
  (let* ((table (make-hash-table :test 'equal)))
    (dolist (line input-lines)
      (if (string-match "^\\(.+\\) bags contain no other bags.$" line)
          (puthash (match-string 1 line) '() table)
        (string-match "^\\(.+\\) bags? contains? \\(.+\\)\\.$" line)
        (let* ((key (match-string 1 line))
               (value (day7-parse-bag-rule-value (match-string 2 line))))
          (puthash key value table))))
    table))

(defun day7-parse-bag-rule-value (bag-list-string)
  (let* ((bag-list (split-string bag-list-string ", " t)))
    (mapcar (lambda (bag-string)
              (progn
                (string-match "^\\([0-9]+\\) \\(.+\\) bags?$" bag-string)
                (list (string-to-number (match-string 1 bag-string)) (match-string 2 bag-string))))
            bag-list)))

(defun day7-find-enclosing-bags-for (inner-bag-name bag-rules)
  (let* ((enclosing-bags '()))
    (maphash (lambda (key value)
               (progn
                 (when (some (lambda (x) x) (mapcar (lambda (v) (equal (elt v 1) inner-bag-name)) value))
                   (setq enclosing-bags (cons key enclosing-bags))))) bag-rules)
    (let* ((second (mapcar (lambda (x) (day7-find-enclosing-bags-for x bag-rules)) enclosing-bags))
           (third (apply 'append (cons enclosing-bags second)))
           (fourth (delete-dups third)))
      fourth)))

(defun day7-number-of-bags-required-inside (outer-bag-name bag-rules)
  (let* ((bag-rule (gethash outer-bag-name bag-rules)))
    (if (equal 0 (length bag-rule))
        0
      (apply '+ (mapcar (lambda (x)
                          (progn
                            (+ (car x) (* (car x) (day7-number-of-bags-required-inside (elt x 1) bag-rules)))))
                        bag-rule)))))

(defun day7-part1 (input)
    (let* ((input-lines (split-string input "\n" t))
           (bag-rules (day7-parse-bag-rules input-lines))
           (list-of-enclosing-bags (day7-find-enclosing-bags-for "shiny gold" bag-rules)))
      (message "bags are %s" list-of-enclosing-bags)
      (length list-of-enclosing-bags)))

(defun day7-part2 (input)
  (let* ((input-lines (split-string input "\n" t))
         (bag-rules (day7-parse-bag-rules input-lines)))
    (day7-number-of-bags-required-inside "shiny gold" bag-rules)))

(message "Number of bag colours that can eventually contain at least one shiny gold bag: %d" (day7-part1 day7-input))
(message "Number of bags required inside a single shiny gold bag: %d" (day7-part2 day7-input))
