...
(setq day2-puzzle-input-example "1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc")

(setq day2-puzzle-input "9-10 b: bbktbbbxhfbpb
2-10 x: xxnxxxwxxsx
5-7 w: ghwwdrr
4-6 z: nzzjzk
7-8 s: szsssswfs
...
(defun day2-part1 (input)
  (let ((input-list (split-string input "\n" t))
        (total 0))
    (dolist (str input-list)
      (string-match "\\([0-9]+\\)-\\([0-9]+\\) \\(.\\): \\(.+\\)" str)
      (let ((low_num (string-to-number (match-string 1 str)))
            (high_num (string-to-number (match-string 2 str)))
            (char (string-to-char (match-string 3 str)))
            (password (match-string 4 str)))
        (setq blah-matches (safe-length (seq-filter (lambda (x) (char-equal x char)) password)))
        (when (and (<= low_num blah-matches) (>= high_num blah-matches))
          (setq total (+ total 1)))))
    (message "Number of valid passwords: %d" total)))
  ...
  (let ((input-list (split-string input "\n" t))
    ...
    (dolist (str input-list)
      ...
      (let ((low_num (string-to-number (match-string 1 str)))
            ...
            (char (string-to-char (match-string 3 str)))
            (password (match-string 4 str)))
        (when (xor (and (<= low_num (length password)) (char-equal char (string-to-char (substring password (1- low_num)))))
                   (and (<= high_num (length password)) (char-equal char (string-to-char (substring password (1- high_num))))))
          (setq total (1+ total)))))
    (message "Number of valid passwords: %d" total)))

(day2-part1 day2-puzzle-input)
(day2-part2 day2-puzzle-input)
