...
(defun day6-part1 (input)
  (apply #'+ (mapcar (lambda (y) (length (delete-dups (seq-remove (lambda (x) (equal x ?\n)) y)))) (split-string input "\n\n" t))))

(defun day6-part2 (input)
  (let* ((input1 (split-string input "\n\n" t))
         (input2 (mapcar (lambda (x) (split-string x "\n" t)) input1))
         (input3 (mapcar (lambda (group) (seq-reduce (lambda (x y) (cl-intersection x (string-to-list y))) (cdr group) (string-to-list (car group)))) input2)))
    (apply #'+ (mapcar #'length input3))))

(message "Sum of counts for each group: %d" (day6-part1 day6-input))
(message "Sum of questions for each group everyone answered yes to: %d" (day6-part2 day6-input))
