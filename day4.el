...
(setq day4-input-example "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
(setq day4-input "byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
...
(defun day4-create-hash-table (input)
  (let ((hash-table (make-hash-table :test 'equal)))
    (seq-do (lambda (x) (let ((y (split-string x ":")))
                          (puthash (car y) (elt y 1) hash-table)))
            input)
    hash-table))

(defun day4-is-valid-passport (passport)
  (and
   (gethash "byr" passport nil)
   (gethash "iyr" passport nil)
   (gethash "eyr" passport nil)
   (gethash "hgt" passport nil)
   (gethash "hcl" passport nil)
   (gethash "ecl" passport nil)
   (gethash "pid" passport nil)
   ))

(defun day4-is-more-valid-passport (passport)
  (let ((byr (string-to-number (gethash "byr" passport)))
        (iyr (string-to-number (gethash "iyr" passport)))
        (eyr (string-to-number (gethash "eyr" passport)))
        (hgt (gethash "hgt" passport)))
    (and
     (and (<= 1920 byr) (>= 2002 byr))
     (and (<= 2010 iyr) (>= 2020 iyr))
     (and (<= 2020 eyr) (>= 2030 eyr))
     (or (and (string-match "^\\([0-9]+\\)cm$" hgt) (<= 150 (string-to-number (match-string 1 hgt))) (>= 193 (string-to-number (match-string 1 hgt))))
         (and (string-match "^\\([0-9]+\\)in$" hgt) (<= 59 (string-to-number (match-string 1 hgt))) (>= 76 (string-to-number (match-string 1 hgt)))))
     (string-match "^#[0-9a-f]\\{6\\}$" (gethash "hcl" passport))
     (string-match "^amb\\|blu\\|brn\\|gry\\|grn\\|hzl\\|oth$" (gethash "ecl" passport))
     (string-match "^[0-9]\\{9\\}$" (gethash "pid" passport)))))

(defun day4-part1 (input)
  (let* ((input1 (split-string input "\n\n" t))
         (input2 (mapcar (lambda (x) (split-string x)) input1))
         (passports (mapcar #'day4-create-hash-table input2)))
    (length (seq-filter #'day4-is-valid-passport passports))))

(defun day4-part2 (input)
  (let* ((input1 (split-string input "\n\n" t))
         (input2 (mapcar (lambda (x) (split-string x)) input1))
         (passports (mapcar #'day4-create-hash-table input2))
         (initial-valid-passports (seq-filter #'day4-is-valid-passport passports)))
    (length (seq-filter #'day4-is-more-valid-passport initial-valid-passports))))


(message "Number of passports with required fields: %d" (day4-part1 day4-input))
(message "Number of valid passports: %d" (day4-part2 day4-input))
